...
#https://github.com/TheLustriVA/diffusiondb/blob/6d0a01bbe137f6db7fe47677349070c296d5477a/download.py
...
def download(index=1, range_index=0, output="", large=False):
    ...
    """
    baseurl = "https://huggingface.co/datasets/UCSC-VLAA/Recap-DataComp-1B/resolve/main/data/train_data/"
    files_to_unzip = []
    #url = f"{baseurl}train-{index:05}-of-04627.parquet"
    ...
    if range_index == 0:
        ...
        file_path = f"{output}train-{index:05}-of-04627.parquet"
        try:
        ...
        with alive_bar(range_index - index, title="Downloading files") as bar:
            for idx in range(index, range_index):
                url = f"{baseurl}train-{index:05}-of-04627.parquet"
                loop_file_path = f"{output}train-{idx:05}-of-04627.parquet"
...
def main(index=None, range_max=None, output=None, unzip=None, large=None):
    ...
    if index and range_max:
        if range_max - index >= 1999:
            confirmation = "y"#input("Do you have at least 1.7Tb free: (y/n)")
            if confirmation != "y":
                return
