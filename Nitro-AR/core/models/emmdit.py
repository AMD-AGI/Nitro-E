...
from core.models.diffloss import DiffLoss
...
class MMDiTTransformer2DModel(SD3Transformer2DModel):
    ...
    def __init__(
        ...
        self.z_proj_ln = nn.LayerNorm(self.inner_dim, eps=1e-6)
    ...
    def forward(
        ...
        orig_img_patches = patchify(img_feat)

        img_patches = orig_img_patches.clone()
        

        if use_ncl:
            sigmas = torch.rand(img_patches.shape[0], img_patches.shape[1])
            sigmas = sigmas.unsqueeze(-1).to(img_patches.dtype).to(img_patches.device)
            noise = torch.randn_like(img_patches).to(img_patches.dtype).to(img_patches.device)
            img_patches = (1-sigmas) * img_patches + sigmas * noise

        orders = sample_orders(bsz=img_patches.size(0), seq_len=height*width)
        mask = random_masking(img_patches, orders, self.mask_ratio_min, 
        ...
        loss, pred_x0 = self.forward_loss(z=hidden_states, target=orig_img_patches, mask=mask)

        return loss, pred_x0, zs

    def sample_tokens(self, txt_emb, uncond_txt_emb, num_iter=64, cfg=1.0, 
        ...
        tokens = torch.ones(bsz, self.seq_len, 32).cuda().to(txt_emb.dtype)
        ...
        for step in indices:
            ...
            hidden_states = hidden_states[mask_to_pred.nonzero(as_tuple=True)]
